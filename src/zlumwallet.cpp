...
CzLUMWallet::CzLUMWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zlum seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadzLUMSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.ErasezLUMSeed_deprecated()) {
                    LogPrintf("%s: Updated zLUM seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zlum seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zlum wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzLUMWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WritezLUMCount(nCountLastUsed);
    else if (!walletdb.ReadzLUMCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzLUMWallet::Lock()
{
    seedMaster = 0;
}

void CzLUMWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzLUMWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedTozLUM(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzLUMWallet::LoadMintPoolFromDB()
{
...
void CzLUMWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzLUMWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzLUMWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zlumTracker->HasPubcoinHash(pMint.first)) {
...
bool CzLUMWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedTozLUM(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zlumTracker which also adds to database
    pwalletMain->zlumTracker->Add(dMint, true);
...
void CzLUMWallet::SeedTozLUM(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzLUMWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzLUMWallet::GenerateDeterministiczLUM(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzLUMWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzLUMWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
