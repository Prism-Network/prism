...
CzLUMTracker::CzLUMTracker(std::string strWalletFile)
{
...
CzLUMTracker::~CzLUMTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzLUMTracker::Init()
{
...
bool CzLUMTracker::Archive(CMintMeta& meta)
{
...
bool CzLUMTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzLUMTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzLUMTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzLUMTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzLUMTracker::GetSerialHashes()
{
...
CAmount CzLUMTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzLUMTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzLUMTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzLUMTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzLUMTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzLUMTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzLUMTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzLUMTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzLUMTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzLUMTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzLUMTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzLUMTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzLUMTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzLUMTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzLUMTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzLUMTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzLUMTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzlum from DB\n", __func__, listDeterministicDB.size());
    }
