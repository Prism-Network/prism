...
// The zLUM block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zLUM is a hash of the serial
...
{
    //Create an output returning the zLUM that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreatezLUMOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zLUM output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zLUM", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreatezLUMOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zLUM output", __func__);
...
{
    CzLUMTracker* zlumTracker = pwallet->zlumTracker.get();
    CMintMeta meta;
    if (!zlumTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zlumTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a LUM stake is the outpoint
