...
public:
    ...
    bool WritezLUMSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadzLUMSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadzLUMSeed_deprecated(uint256& seed);
    bool ErasezLUMSeed();
    bool ErasezLUMSeed_deprecated();

    bool WritezLUMCount(const uint32_t& nCount);
    bool ReadzLUMCount(uint32_t& nCount);
