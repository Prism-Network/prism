...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::LUM);
    ...
    if (!settings.contains("nAnonymizeBitCoinOneAmount"))
        settings.setValue("nAnonymizeBitCoinOneAmount", 1000);

    nAnonymizeBitCoinOneAmount = settings.value("nAnonymizeBitCoinOneAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeBitCoinOneAmount"))
        SoftSetArg("-anonymizeprismamount", settings.value("nAnonymizeBitCoinOneAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in prism.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeBitCoinOneAmount:
            return QVariant(nAnonymizeBitCoinOneAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case HideOrphans:
            ...
            break;
        case AnonymizeBitCoinOneAmount:
            nAnonymizeBitCoinOneAmount = value.toInt();
            settings.setValue("nAnonymizeBitCoinOneAmount", nAnonymizeBitCoinOneAmount);
            emit anonymizeBitCoinOneAmountChanged(nAnonymizeBitCoinOneAmount);
            break;
        case CoinControlFeatures:
