...
bool ValidateAccumulatorCheckpoint(const CBlock& block, CBlockIndex* pindex, AccumulatorMap& mapAccumulators)
{
    ...
    //It is VERY IMPORTANT that when this is being run and height < v2_start, then zLUM need to be disabled at the same time!!
...
bool GenerateAccumulatorWitness(const PublicCoin &coin, Accumulator& accumulator, AccumulatorWitness& witness, int nSecurityLevel, int& nMintsAdded, string& strError, CBlockIndex* pindexCheckpoint)
{
    ...
    while (pindex) {
        ...
        // 10 blocks were accumulated twice when zLUM v2 was activated
...
map<CoinDenomination, int> GetMintMaturityHeight()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList)
        mapRet.insert(make_pair(denom, nMinimumMaturityHeight));
    return mapRet;
}
