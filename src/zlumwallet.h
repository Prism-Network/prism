...
#ifndef LUM_zLUMWALLET_H
#define LUM_zLUMWALLET_H
...
public:
    CzLUMWallet(std::string strWalletFile);
    ...
    void GenerateDeterministiczLUM(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedTozLUM(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //LUM_zLUMWALLET_H
