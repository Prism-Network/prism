...
void TestSHA1(const std::string &in, const std::string &hlumut) { TestVector(CSHA1(), in, ParseHex(hlumut));}
void TestSHA256(const std::string &in, const std::string &hlumut) { TestVector(CSHA256(), in, ParseHex(hlumut));}
void TestSHA512(const std::string &in, const std::string &hlumut) { TestVector(CSHA512(), in, ParseHex(hlumut));}
void TestRIPEMD160(const std::string &in, const std::string &hlumut) { TestVector(CRIPEMD160(), in, ParseHex(hlumut));}

void TestHMACSHA256(const std::string &hexkey, const std::string &hexin, const std::string &hlumut) {
    std::vector<unsigned char> key = ParseHex(hexkey);
    TestVector(CHMAC_SHA256(&key[0], key.size()), ParseHex(hexin), ParseHex(hlumut));
}

void TestHMACSHA512(const std::string &hexkey, const std::string &hexin, const std::string &hlumut) {
    std::vector<unsigned char> key = ParseHex(hexkey);
    TestVector(CHMAC_SHA512(&key[0], key.size()), ParseHex(hexin), ParseHex(hlumut));
}
...
void TestRFC6979(const std::string& hexkey, const std::string& hexmsg, const std::vector<std::string>& hlumut)
{
    std::vector<unsigned char> key = ParseHex(hexkey);
    std::vector<unsigned char> msg = ParseHex(hexmsg);
    RFC6979_HMAC_SHA256 rng(&key[0], key.size(), &msg[0], msg.size());

    for (unsigned int i = 0; i < hlumut.size(); i++) {
        std::vector<unsigned char> out = ParseHex(hlumut[i]);
