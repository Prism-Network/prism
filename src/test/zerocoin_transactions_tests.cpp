...
BOOST_AUTO_TEST_CASE(zerocoin_spend_test)
{
    ...
    cWallet.zlumTracker = unique_ptr<CzLUMTracker>(new CzLUMTracker(cWallet.strWalletFile));
    ...
    BOOST_CHECK_MESSAGE(receipt.GetStatus() == zLUM_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
    ...
    BOOST_CHECK_MESSAGE(receipt2.GetStatus() == zLUM_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
